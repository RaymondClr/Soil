...
(function() {
    ...
    var itemsArr = collectionToArray(projectItems);
    log(map(itemsArr, function(item) {
        return item.name;
    }));
    var activeComp = getActiveComp();
    if (isCompItem(activeComp)) {
        var layers = collectionToArray(activeComp.layers);
        forEach(layers, function(layer, index) {
            log("".concat(index + 1, " ").concat(layer.name));
        });
    }
}).call(this);

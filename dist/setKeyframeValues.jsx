// Raymond Yan (raymondclr@foxmail.com / qq: 1107677019) - 2024/11/18 17:13:43
...
(function() {
    ...
    function isTwoDSpatialProperty(property) {
    ...
    if (isLayer(selectedLayer)) {
        var targetProperty = selectedLayer.position;
        var activeComp = selectedLayer.containingComp;
        var frameDuration_1 = activeComp.frameDuration;
        var keyframeValues = times(5, function(n) {
            var keyframe = {
                keyTime: n * frameDuration_1 * 8,
                keyValue: [ n * 100, n * 100 ]
            };
            return keyframe;
        });
        if (canSetPropertyValue(targetProperty)) {
            setKeyframeValues(targetProperty, keyframeValues);
        }
    }
    var selectedLayers = getSelectedLayers();
    if (selectedLayers && selectedLayers.length === 2) {
        var _a = selectedLayers.sort(function(layer) {
            return layer.index;
        }), firstLayer = _a[0], secondLayer = _a[1];
        var firstLayerPos = firstLayer.position;
        var secondLayerPos = secondLayer.position;
        if (canSetPropertyValue(secondLayerPos) && isTwoDSpatialProperty(secondLayerPos)) {
            var firstLayerKeys = getKeyframeValues(firstLayerPos);
            setKeyframeValues(secondLayerPos, firstLayerKeys);
        }
    }
}).call(this);
